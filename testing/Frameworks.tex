...
\section{Introduction}
  ...
  XCTest is essentially Apple's version of OCUnit/SenTestingKit and is quite an
  old library,
  ...
  not have any support for matchers or context based tests furthermore
  asynchronous testing is only a recent addition. These 
  limitations that are fine when testing in a \textit{Test Last}
  situation (referred to as Unit Testing) and also work fine for simple
...
\section{BDD syntax}
  ...
  effectively to the other part's. 

  As such we end up with a series of logical \textit{contexts} that define
  area's of logic, both in terms of business logic (that managers
  ...
  This is a trivial example but shows the general idea.
  ...
  This is somewhat contrived but it illustrates the general idea.
  \\We can break this down further by driving out the \textbf{Acceptance
  Criteria}, these allow us to know when the task can be considered
  ...
  This structure also works well for describing the integration tests
  that QA will need to do to ensure that we have met the requirements.
  We will come back to this in \hyperref[sec:integration tests]{Functional/Integration tests}
...
\section{Specs as tickets and estimation}
  ...
  for the story to be accepted and thus we need to build up our code under 
  test to meet these.
  ...
  understood across the levels of the business but in terms of
  development we are going to need to go deeper and create specs
   for the development teams, really it's these technical specs,
  that we can consider as a sub category of a BDD User Story that we
  ...
  to set a BOOL flag that indicates that the icon needs display.
  ...
  Looking into the first one we find out that there are a couple of
  conditions that determine wether or not cross platform sync is
  ...
  small reusable and extensible helper class that can capture the logic
  we need and return a BOOL value that determines wether or not to
  display the icon, the class responsible for displaying the icon can
  the use this new helper class, and we will give it the class as a
